...
public class Cadenas : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (collision.gameObject.GetComponent<ControleJoueur>().getInventaire().Contains(CollectionnableEnum.Cle_bronze))
            {
                if (!isCle1)
                ...
                collision.gameObject.GetComponent<ControleJoueur>()._inventaire.Remove(CollectionnableEnum.Cle_bronze);
            }
            if (collision.gameObject.GetComponent<ControleJoueur>().getInventaire().Contains(CollectionnableEnum.Cle_argent))
            {
                if(!isCle2)
            ...
            if (collision.gameObject.GetComponent<ControleJoueur>().getInventaire().Contains(CollectionnableEnum.Cle_or))
            {
                if (!isCle3)
                ...
                collision.gameObject.GetComponent<ControleJoueur>()._inventaire.Remove(CollectionnableEnum.Cle_or);
